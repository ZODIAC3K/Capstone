...
export async function POST(request: NextRequest) {
    ...
    if (Object.keys(await request.json()).length > 5) {
        return NextResponse.json({ error: 'Body length is more than 5 variables' }, { status: 400 })
    }

    const { offer_discount, title, description, applicable_on, end_at, code } = await request.json()
...
export async function PATCH(request: NextRequest) {
    ...
    const { offer_id, offer_discount, title, description, applicable_on, end_at, code } = await request.json()
    if (!offer_id) {
