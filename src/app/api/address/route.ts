...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            const updatedUser = await UserModel.findById(auth.userId).populate('savedAddress').session(session)
    ...
    try {
        ...
        try {
            ...
            if (!body.addressId || !body.address) {
                await session.abortTransaction()
                return NextResponse.json({ error: 'Address ID and details are required' }, { status: 400 })
            }

            // Validate address fields
            if (!body.address.firstLine || !body.address.pincode || !body.address.city || !body.address.state) {
                await session.abortTransaction()
                return NextResponse.json({ error: 'Missing required address fields' }, { status: 400 })
            }
            ...
            if (body.default === true) {
                ...
                await AddressModel.updateMany({ user_id: auth.userId }, { default: false }, { session })
            }
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        try {
            ...
            const addressToDelete = await AddressModel.findById(body.addressId).session(session)
            ...
            const addressIndex = user.savedAddress.findIndex((addr: any) => addr.toString() === body.addressId)
            ...
            // Delete the address
            await AddressModel.findByIdAndDelete(body.addressId).session(session)
            ...
            return NextResponse.json(
                {
                    ...
                    newDefaultAddressId: wasDefault && user.savedAddress.length > 0 ? user.savedAddress[0] : null
                },
                { status: 200 }
            )
        } catch (error: any) {
...
export async function GET(request: NextRequest) {
    try {
        ...
        try {
            ...
            return NextResponse.json({ error: error.message || 'Failed to fetch addresses' }, { status: 400 })
        } finally {
            session.endSession()
        }
    } catch (error: any) {
