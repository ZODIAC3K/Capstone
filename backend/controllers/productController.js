...
async function fetchProducts(req, res, next) {
    try {
        ...
        options.skip = options.skip ? parseInt(options.skip) * parseInt(options.limit) : 0 // If no skip / page specified take 0 (start from the first document).
...
function productById(req, res, next) {
    try {
        ...
        Product.findById(id)
            .then(async (product) => {
                product = await populateAllAttributes(product, productDetailsSchema)
    ...
    try {
        const createdStockDetails = await Promise.all(
            stockDetailsArray.map(async (stockDetailData) => {
                const { color, size, amount, quantity, images } = stockDetailData
                const imageArray = []
