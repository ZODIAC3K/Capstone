...
const { OrderDetails, stockDetail, CouponDetails, OfferDetails } = require('../models')
            ...
            stockDetail
                ...
                .then(async (stock_details) => {
                    ...
                    if (offer_used[idx] !== 'null' && offer_used[idx] !== undefined) {
                            ...
                            if (!offer) {
                                ...
                                if (offer.applicable_on.includes(product_id)) {
                                    if (offer.end_at < Date.now()) {
                                        ...
                                        each_product_amount =
                                            stock_details.amount - stock_details.amount * (offer.offer_discount / 100) // amount - (amount * offer_discount/100)
                                        product_ordered_amount = each_product_amount * parseInt(quantity_ordered[idx]) //  (quantity * amount)
                                        final_amount += product_ordered_amount // total amount after applying offer on each product
                                    }
                                } else {
                                    Errors.push({
                                        offer_used: offer_used[idx],
                                        message: 'Offer is not applicable on this product',
                                        status: false
                                    })
                                }
                            }
                        ...
                        product_ordered_amount = each_product_amount * parseInt(quantity_ordered[idx])
                        final_amount += product_ordered_amount // total amount after applying offer on each product
                    }
                })
                .catch((err) => {
...
async function razorpayPaymentVerificationAndOrderUpdate(req, res, next) {
    ...
    const expectedSignature = crypto.createHmac('sha256', RAZORPAY_SECRET_KEY).update(check.toString()).digest('hex')
            ...
            stockDetail
                ...
                .then(async (stock_details) => {
                    if (!stock_details) {
                        ...
                        stock_details.quantity = stock_details.quantity - order.quantity_ordered[idx]
                        stock_details.save()
                    }
                })
        })
...
async function updateOrder(req, res) {
    ...
    try {
        const updatedOrder = await OrderDetails.findByIdAndUpdate(req.params.id, req.body, {
            new: true
        })
